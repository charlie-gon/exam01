      	...
      	function makeData(){
			...
			//체크한 행만 전송할 데이터 만들기. 은행계좌가 입력된 경우만 포함함.
      		$("[name='selId']:checked").each(function(i, checkbox){
				...
				//객체에 담기
				obj["slipAmount"] = salary*1 + commission;  // 급여 + 상여금. *1 해주면 parseInt와 동일한 효과!
				obj["salDt"] = mon;               // 급여년월 (현재년월)
			...
			//객체를 string으로 변환
      		...
      		$.ajax({
      			...
      			success:function(response){
      				//alert(response.cnt + "건 삽입 완료"); // Controller에서 넘겨주는 변수 cnt
      			}
      		});
      	}
      </script>
